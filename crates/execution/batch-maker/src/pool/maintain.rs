    ...
    Tasks: TaskSpawner + 'static,
{
    ...
    loop {
        ...
        let depth = pending.number.abs_diff(pool_info.last_seen_block_number);
        if depth > max_update_depth {
            maintained_state = MaintainedPoolState::Drifted;
            debug!(target: "txpool", ?depth, "skipping deep canonical update");
            let info = BlockInfo {
                last_seen_block_hash: parent.hash(),
                last_seen_block_number: parent.number,
