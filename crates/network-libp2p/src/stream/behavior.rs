...
pub(crate) struct StreamBehavior {
    /// The chain-namespaced per-role sync protocol, advertised by each handler.
    sync: StreamProtocol,
...
impl StreamBehavior {
    /// Create a new stream behavior whose handlers advertise the chain-namespaced
    /// per-role `sync` protocol, cloned into each new connection handler; see
    /// [`stream_protocol`](crate::types::stream_protocol).
...
impl NetworkBehaviour for StreamBehavior {
    ...
    fn handle_established_inbound_connection(
        ...
        Ok(StreamHandler::new(self.sync.clone()))
    }
...
mod tests {
    ...
    // the sole stream protocol a node registers. Protocol naming is locked in
    // `kad::tests::test_network_type_protocol_names`.
