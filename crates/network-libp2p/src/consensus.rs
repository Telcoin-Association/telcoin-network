...
/// - `req_res`: Point-to-point request-response for sync negotiation
/// - `peer_manager`: Connection management and peer scoring
/// - `kademlia`: Distributed hash table for peer discovery
/// - `stream_sync`: Stream-based bulk data transfer for state sync
    ...
    C: Codec + Send + Clone + 'static,
{
    ...
    pub(crate) stream_sync: StreamBehavior,
}
    ...
    DB: Database,
{
    ...
    pub(crate) fn new(
        ...
        let stream_sync = StreamBehavior::new();
        Self { gossipsub, req_res, peer_manager, kademlia, stream_sync }
    }
}
    ...
    Events: TnSender<NetworkEvent<Req, Res>>,
{
    ...
    /// Pending sync stream requests awaiting stream establishment.
    ///
    /// Maps `request_id` to the oneshot channel for returning the established stream.
    /// The `request_id` is generated by `sync_request_counter` and used to correlate
    /// the stream with the original `OpenStream` command.
    pending_sync_streams: HashMap<u64, oneshot::Sender<Result<Stream, SyncStateError>>>,
    /// Counter for generating unique sync request IDs.
    ///
    /// Each call to `OpenStream` increments this counter to produce a unique ID
    /// that correlates the command with the resulting stream event.
    sync_request_counter: u64,
}
    ...
    Events: TnSender<NetworkEvent<Req, Res>> + Send + 'static,
{
    ...
    pub fn new(
        ...
        Ok(Self {
            ...
            task_spawner,
            node_record,
            pending_sync_streams: HashMap::new(),
    ...
    async fn process_event(
        ...
        match event {
            SwarmEvent::Behaviour(behavior) => match behavior {
                ...
                TNBehaviorEvent::StreamSync(event) => self.process_stream_sync_event(event)?,
            },
    ...
    fn process_command(&mut self, command: NetworkCommand<Req, Res>) -> NetworkResult<()> {
        match command {
            ...
            NetworkCommand::OpenStream { peer, resource_id, expected_hash, reply } => {
                ...
                let peer_id = match self.swarm.behaviour().peer_manager.auth_to_peer(peer) {
                    ...
                    None => {
                        ...
                        let _ = reply.send(Err(SyncStateError::PeerMissing));
                        return Ok(());
                    }
                };

                // Generate a unique request ID for correlating the stream event
                self.sync_request_counter += 1;
                let request_id = self.sync_request_counter;
                ...
                debug!(
                    ...
                    resource_id,
                    request_id,
                    "opening sync stream to peer"
                );

                // Store the reply channel to complete when stream is established
                self.pending_sync_streams.insert(request_id, reply);

                // Request the stream behavior to open a stream to this peer
                self.swarm.behaviour_mut().stream_sync.open_stream(
                    peer_id,
                    request_id,
                    StreamHeader { resource_id, request_id, expected_hash },
                );
            }
        }
    ...
    /// Process events from the stream sync behavior.
    ///
    /// This handles inbound and outbound stream events for bulk data transfer.
    fn process_stream_sync_event(&mut self, event: StreamEvent) -> NetworkResult<()> {
        match event {
            StreamEvent::InboundStream { peer, stream, header } => {
                debug!(
                    target: "network",
                    ?peer,
                    resource_id = header.resource_id,
                    request_id = header.request_id,
                    "inbound sync stream received"
                );
                // Forward to application layer via event_stream
                if let Some(bls) = self.swarm.behaviour().peer_manager.peer_to_bls(&peer) {
                    if let Err(e) = self.event_stream.try_send(NetworkEvent::InboundSyncStream {
                        peer: bls,
                        stream,
                        header,
                    }) {
                        error!(target: "network", ?e, "failed to forward inbound sync stream");
                    }
                } else {
                    warn!(target: "network", ?peer, "received inbound sync stream from unknown peer");
                }
            }
            StreamEvent::OutboundStream { peer, stream, request_id, header: _ } => {
                debug!(
                    target: "network",
                    ?peer,
                    request_id,
                    "outbound sync stream established"
                );

                // Find and complete the pending sync stream request
                if let Some(reply) = self.pending_sync_streams.remove(&request_id) {
                    ...
                    warn!(
                        target: "network",
                        request_id,
                        "outbound stream for unknown sync request - dropping stream"
                    );
                    drop(stream);
                }
            }
            StreamEvent::OutboundFailure { peer, request_id, error } => {
                warn!(
                    target: "network",
                    ?peer,
                    request_id,
                    ?error,
                    "outbound sync stream failed"
                );

                // Find and complete the pending sync stream request with error
                if let Some(reply) = self.pending_sync_streams.remove(&request_id) {
                    let _ = reply.send(Err(SyncStateError::StreamFailed(error)));
                }
            }
        }
        Ok(())
