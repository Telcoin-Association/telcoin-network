...
- **Statement:** If `current_committee_keys[bls] == Some(peer_id)`, then `peer_id ∈ current_committee` and `peers[peer_id].is_trusted == true`. The promotion helper `AllPeers::promote_committee_member` is the single funnel that establishes this triple, called from both `new_epoch` (BLS resolved up front) and `upsert_peer` via two gates: the **unresolved arm** (`current_committee_keys[bls] == Some(&None)` — BLS later resolved by a kad lookup) and the **rotation arm** (`current_committee_keys[bls] == Some(&Some(other_pid))` with `other_pid != peer_id` — a sitting validator's libp2p PeerId changed mid-epoch). In the rotation arm, `upsert_peer` removes `other_pid` from `current_committee` *before* calling `promote_committee_member`, so the stale CVV mapping is evicted in the same step that the new PeerId is promoted; the orphan record itself is left alone (its `is_trusted == true` is preserved per INV-045, and `rebind_bls` has already cleared its BLS).
- **Why:** The trio (`current_committee_keys`, `current_committee`, `is_trusted`) must stay consistent so that `is_peer_validator` and `prune_connected_peers` (INV-033) and the pruning protection (INV-045) all see the same committee membership. Splitting the promotion logic between two call sites was the original source of F-1: a kad-driven `add_known_peer` could refresh `bls_index` without updating committee state, so the next read of `is_peer_validator` would lie. The rotation arm is rare in practice — `Config::libp2p_keypair` derives the network keypair deterministically from `BLS + seed` (`config/src/keys.rs`), so a routine restart preserves the PeerId and hits the `Some(&None)` arm at most; only an operator-driven keypair seed change produces a fresh PeerId for an already-resolved committee BLS.
- **Enforced at:** `all_peers.rs` (`promote_committee_member` writes all three; `new_epoch` and `upsert_peer` are the only callers; `upsert_peer`'s rotation arm performs the `current_committee.remove(&other_pid)` eviction). Regression tests: `tests/peers.rs::new_epoch_resolves_via_index`, `tests/peers.rs::committee_member_rotation_promotes_new_peer_id`.
- **Violation impact:** A "resolved" committee entry with a peer that is not trusted or not in `current_committee` would let `prune_banned_peers` evict the validator (defeats INV-045) or let `process_penalty` ban it (defeats INV-002). Without the rotation arm specifically, a validator that rotates its PeerId mid-epoch is silently locked out for the remainder of the epoch — the new PeerId is reachable via `bls_index` but is treated as an untrusted NVV.
