...
impl HdxHeader {
    ...
    pub(super) fn from_data_header<const KSIZE: usize, S: BuildHasher + Default>(
        ...
        Self {
            ...
            bucket_elements: HdxIndexDirectIO::<KSIZE, S>::BUCKET_ELEMENTS as u16,
            bucket_size: HdxIndexDirectIO::<KSIZE, S>::BUCKET_SIZE as u16,
            buckets: HdxIndexDirectIO::<KSIZE, S>::INITIAL_BUCKETS as u32,
    ...
    pub(super) fn load_header<F: Read + Seek + ?Sized>(
        hdx_file: &mut F,
    ...
    pub(super) fn write_header<F: Write + Seek + ?Sized>(
        &mut self,
        hdx_file: &mut F,
    ) -> Result<(), io::Error> {
        hdx_file.rewind()?;
...
/// An hdx (index) file.  This contains the hash buckets for lookups.
/// This file is not a log file and the header and buckets will change in place over time.
/// This data in the file will be followed by a CRC32 checksum value to verify it.
/// This version uses caches and Direct IO instead of mmap file APIs.  Keeping for
/// now but generally the mmap (default) version is faster and should be used.
/// Consider this depricated.
#[derive(Debug)]
pub struct HdxIndexDirectIO<
...
impl<const KSIZE: usize, S: BuildHasher + Default> HdxIndexDirectIO<KSIZE, S> {
    ...
    pub fn open_hdx_file<P: AsRef<Path>>(
        ...
        hasher_builder: S,
        read_only: bool,
    ) -> Result<HdxIndexDirectIO<KSIZE, S>, LoadHeaderError> {
...
impl<const KSIZE: usize, S: BuildHasher + Default> Drop for HdxIndexDirectIO<KSIZE, S> {
    fn drop(&mut self) {
...
impl<const KSIZE: usize, S: BuildHasher + Default> Index<B256, u64> for HdxIndexDirectIO<KSIZE, S> {
...
mod tests {
    ...
    fn test_archive_hdx_index() {
        ...
        let mut idx: HdxIndexDirectIO = HdxIndexDirectIO::open_hdx_file(
    ...
    fn test_archive_hdx_index_mmap_backend() {
        ...
        let open = |read_only: bool| -> HdxIndexDirectIO {
    ...
    fn test_archive_hdx_index_geometry_mismatch() {
        ...
        let builder = BuildHasherDefault::<FxHasher>::default();
        let res = HdxIndexDirectIO::<16>::open_hdx_file(
            tmp_path.join("index.hdx"),
            &data_header,
            builder,
            false,
