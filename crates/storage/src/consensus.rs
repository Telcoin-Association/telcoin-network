...
impl ConsensusChain {
    ...
    pub async fn consensus_header_by_digest(
        ...
        // A sealed epoch whose pack is present but unreadable propagates as `Err` from
        // `get_static_if_present`; only a genuinely absent epoch (files never on disk) falls
        // through to the staging fallback / `Ok(None)`. (The current-epoch branch above still
        // collapses pack-internal read failures to `None`: the in-memory channel wrapper types
        // them away before this layer.)
