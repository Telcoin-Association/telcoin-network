...
use tn_storage::{
    ...
    open_db,
    tables::{
        CertificateDigestByOrigin, CertificateDigestByRound, Certificates, EpochCerts,
        EpochRecords, LastProposed, NodeBatchesCache, Payload, Votes,
    },
...
use tn_types::{
    ...
    Database as TNDatabase, Epoch, EpochCertificate, EpochRecord, EpochVote, Multiaddr,
    ...
    if let Some(block) = reth_env.finalized_header()? {
        ...
        for current in blocks {
            ...
            if last_round != Some(round) {
                ...
                let leader = if let Ok(Some(block)) =
                    consensus_chain.consensus_header_by_digest(Some(epoch), consensus_digest).await
                {
                    block.sub_dag.leader.origin().clone()
                } else {
                    return Err(eyre::eyre!("missing consensus block"));
                };
    ...
    DB: TNDatabase,
{
    ...
    pub(crate) async fn run(&mut self) -> eyre::Result<()> {
        ...
        // Flush and persist any consensus data to epoch pack file.
    ...
    async fn orphan_batches<QuorumWaiter: QuorumWaiterTrait>(
        ...
        // We can not await while using the db iter so capture the digest and filter out the ones that were processed.
    ...
    fn open_epoch_pack(&mut self, committee: Committee) -> eyre::Result<()> {
        ...
        self.consensus_chain.new_epoch(previous_epoch_rec, committee)?;
        Ok(())
    }
    ...
    async fn run_epoch(
        ...
        self.open_epoch_pack(committee.clone())?;
        ...
        if let Some(last_consensus) = self
            .consensus_chain
        ...
        if need_join {
            ...
            while let Ok(_output) = consensus_output.try_recv() {
                /*XXXXif current_epoch == output.sub_dag().leader_epoch() {
