...
use tn_storage::{
    ...
    tables::{
        ...
        ConsensusBlocks, EpochRecords, LastProposed, NodeBatchesCache, Payload, Votes,
    },
    ConsensusStore, DatabaseType, EpochStore as _,
};
use tn_types::{
    gas_accumulator::GasAccumulator, Batch, BatchValidation, BlockHash, BlsPublicKey,
    CertifiedBatch, CommittedSubDag, Committee, CommitteeBuilder, ConsensusOutput,
    Database as TNDatabase, EngineUpdate, Epoch, EpochRecord, Hash, Multiaddr, NetworkPublicKey,
...
#[path = "epoch_vote_collector.rs"]
mod epoch_vote_collector;
    ...
    DB: TNDatabase,
{
    ...
    pub(crate) async fn run(&mut self) -> eyre::Result<()> {
        ...
        epoch_vote_collector::spawn_epoch_vote_collector(
            self.consensus_db.clone(),
            self.consensus_bus.clone(),
