...
impl WorkerNetworkHandle {
    ...
    async fn request_batches_from_peer(
        ...
        //
        //
        // TODO: review timeout as arg vs const here
        //
        //
        let res = self.handle.send_request(request.clone(), peer).await?;
        let res =
            tokio::time::timeout(timeout, res).await.map_err(|_| NetworkError::Timeout)???;
        ...
        let batches = tokio::time::timeout(timeout, async {
            self.read_and_validate_batches(&mut stream, &batch_digests).await
        })
        .await
    ...
    async fn read_and_validate_batches(
        ...
        // read chunk count
        let batch_count = super::stream_codec::read_chunk_count(stream)
            .await
        ...
        let mut received_digests = HashSet::with_capacity(batch_count);
        ...
        for i in 0..batch_count {
            ...
            // Validate: was this batch requested?
            if !requested_digests.contains(&batch_digest) {
                return Err(NetworkError::ProtocolError(format!(
    ...
    pub(crate) async fn request_batches(
        ...
        for peer in peers {
            ...
            match self
                .request_batches_from_peer(peer, digests_to_request, Duration::from_secs(10))
                .await
            {
                Ok(batches) => {
                    for batch in batches {
    ...
    pub(crate) fn generate_batch_request_id(&self, batch_digests: &Vec<BlockHash>) -> B256 {
