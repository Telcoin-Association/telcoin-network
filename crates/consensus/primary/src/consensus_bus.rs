...
use crate::{
    ...
    synchronizer::NewCertificateMessage, RecentBlocks,
};
...
struct ConsensusBusInner {
    ...
    new_certificate_acceptor: MeteredMpscChannel<NewCertificateMessage>,
...
impl ConsensusBus {
    ...
    pub fn new_with_recent_blocks(recent_blocks: u32) -> Self {
        ...
        let new_certificate_acceptor = channel_with_total_sender(
            CHANNEL_CAPACITY,
    ...
    /// Can only be subscribed to once.
    pub fn new_certificate_acceptor(&self) -> &impl TnSender<NewCertificateMessage> {
        &self.inner.new_certificate_acceptor
    }
