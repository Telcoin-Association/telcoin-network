    ...
    DB: Database,
{
    ...
    /// Use the newly verified certificate to update pending state. Forward this certificate any any unlocked certificates to the CertificateAcceptor.
    ...
    pub async fn run(mut self) -> CertificateResult<()> {
        ...
        let mut new_unverified_cert = self.consensus_bus.new_certificate_acceptor().subscribe();
            ...
            tokio::select! {
                ...
                Some(command) = pending_cert_commands_rx.recv() => {
                    match command {
                        ...
                        PendingCertCommand::ProcessNewVerifiedCertificate { certificate } => {
