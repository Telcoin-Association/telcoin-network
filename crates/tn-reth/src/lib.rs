...
impl RethEnv {
    ...
    pub fn create_consensus_registry_genesis_accounts(
        ...
        // The BlsG1 proof-of-possession library is a native precompile registered by the EVM factory
        // at `BLS_G1_PRECOMPILE_ADDRESS`, so there is nothing to deploy at genesis: the registry is
        // linked directly against the precompile address. The registry constructor's PoP verification
        // `delegatecall`s this address and is served natively by the precompile, which shares the
        // exact `blst` verification the consensus layer uses to produce the proofs (so the on-chain
        // check cannot drift from the off-chain encoding).
...
mod tests {
    ...
    /// Guards the committee backfill path in `block.rs::shuffle_new_committee`: when there are fewer
    /// strictly-active validators than the committee size, the shuffle must backfill from the
    /// `PendingExit` pool so the next committee still reaches the required size. Five genesis
    /// validators form a committee of 5; two begin exiting, leaving 3 active + 2 pending-exit. Since
    /// `committeeSize (5) > active (3)`, every subsequent committee must include the two exiting
    /// validators - otherwise `concludeEpoch` would revert on its committee-size check.
    #[tokio::test]
    async fn test_committee_backfill_from_pending_exit() -> eyre::Result<()> {
        ...
        // with active(3) < committeeSize(5), the backfill must keep every committee full and include
        // the two pending-exit validators
