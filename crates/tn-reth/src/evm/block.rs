...
//! rebuilds the ctx from the sealed header — identical up to `rewards_counter`, which is the live
//! shared counter rather than header-derived (see the `evm/config.rs` module docs for the replay
//! caveat and the `block.body.withdrawals` reconstruction follow-up). The RNG draw order inside
//! the shuffle is consensus-critical: any refactor that reorders the draws selects a different
//! committee.
