...
//! `tn_reth.forwarded_batches_shed_total` and `tn_reth.forwarded_txns_abandoned_total`. One
...
/// counted as lost here were accepted by this node's RPC and never delivered to a validator.
///
...
impl ForwarderMetrics {
    /// Register every forwarder series (shed, abandoned, queued, one dropped series per
    /// [`ForwardDropReason`], and the rejection-overridden series) with the current recorder
