...
| `mix_hash` | Computed in `crates/engine/src/payload_builder.rs` via `ConsensusOutput::prev_randao` (`tn-types` `primary/output.rs`), which is epoch-gated on `prevrandao_seed_active` (legacy `output_digest ^ batch_digest` (plain `output_digest` when no batches) before the fork epoch, `keccak256("TN_PREVRANDAO_V1" \|\| epoch seed chain value \|\| consensus block number \|\| batch index)` with the latter two as little-endian `u64` from the fork epoch on). Exposed as `prevrandao` (EIP-4399). |
