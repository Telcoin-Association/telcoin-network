...
| `extra_data` | Empty for a normal block. For an epoch-closing block: the 32-byte epoch seed chain value as of the closing commit. The replay path (`context_for_block` in `src/evm/config.rs`) accepts only length 0 or 32 and errors on anything else. |
...
- Epoch-close randomness is the epoch seed chain value as of the closing commit, folded per
  commit in consensus (`tn-types` `primary/output.rs`) and carried in the payload. It
