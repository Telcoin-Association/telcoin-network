...
/// at slot 0.
pub(crate) fn amount_slot(recipient: Address) -> U256 {
    let mut buf = [0u8; 64];
...
pub(crate) fn timestamp_slot(recipient: Address) -> U256 {
    let mut buf = [0u8; 64];
    buf[12..32].copy_from_slice(recipient.as_slice());
...
pub(crate) fn allowance_slot(owner: Address, spender: Address) -> U256 {
...
pub(crate) fn nonce_slot(owner: Address) -> U256 {
    let mut buf = [0u8; 64];
