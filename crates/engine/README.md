...
'*.rs' crates/`, `rg 'digest\(\) *[=!]='` over `crates/` (the pack reader writes its check as `!=`,
so an `==`-only pattern misses it), and a full read of `execute_consensus_output`. The only
positional check those turn up is the pack reader above.
