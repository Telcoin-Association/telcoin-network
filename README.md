...
- On a fresh-start auto-restore, `--snapshot-min-epoch N` (env `TN_SNAPSHOT_MIN_EPOCH`) refuses any snapshot whose resolved epoch is below `N`. A bucket cannot forge a snapshot but it can serve an older, still-valid one, so the floor bounds how far back a withholding bucket can push a fresh node's starting point. Manual `restore` has no floor — pin `--epoch` instead.
