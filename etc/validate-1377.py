...
try:
    if PHASE == "clippy":
        ...
        run("slot-core-default", command, required=False)
        run("slot-core-adiri", test_command("tn-types", ["--lib"], "worker::batch_slot", True), required=False)
        ...
        run("slot-worker-default", worker_command, required=False)
        run("slot-worker-adiri", test_command("tn-worker", ["--lib"], "worker::slot_tests::", True), required=False)
        builder_command = test_command("tn-batch-builder", ["--test", "it"], "native_ack_retains_transactions_")
        run("slot-builder-default", builder_command, required=False)
        run("slot-builder-adiri", test_command("tn-batch-builder", ["--test", "it"], "native_ack_retains_transactions_", True), required=False)
        admission_command = test_command("tn-reth", ["--lib"], "native_admission_rejects_delegated_sender_")
        run("slot-admission-default", admission_command, required=False)
        run("slot-admission-adiri", test_command("tn-reth", ["--lib"], "native_admission_rejects_delegated_sender_", True), required=False)
        if any(not check["passed"] for check in REPORT["checks"]):
            raise RuntimeError("Native regression checks failed; all baseline results were collected")
