...
# Committee worker-list fork epoch for the e2e lanes (#554). Same shape as the seed-signature
...
#   TN_COMMITTEE_WORKERS_FORK_EPOCH=1 make test-epochs
# Only test-utils builds consult it (tn_types::forks::committee_workers_fork_epoch_override).
# Set only on name-filtered nextest lines, never a bare --workspace run: tn-types'
# committee_workers_override_is_inert_when_unset requires a process WITHOUT the variable.
TN_COMMITTEE_WORKERS_FORK_EPOCH ?= 4294967295

# run restart integration tests
test-restarts: build-e2e-bin
	TN_BIN_PATH="$(E2E_BIN)" TN_SEED_SIGNATURE_FORK_EPOCH=$(TN_SEED_SIGNATURE_FORK_EPOCH) TN_COMMITTEE_WORKERS_FORK_EPOCH=$(TN_COMMITTEE_WORKERS_FORK_EPOCH) cargo nextest run --run-ignored all test_restarts ;
...
test-epochs: build-e2e-bin
	TN_BIN_PATH="$(E2E_BIN)" TN_SEED_SIGNATURE_FORK_EPOCH=$(TN_SEED_SIGNATURE_FORK_EPOCH) TN_COMMITTEE_WORKERS_FORK_EPOCH=$(TN_COMMITTEE_WORKERS_FORK_EPOCH) cargo nextest run -p e2e-tests --test it --run-ignored all test_epoch ;

# run e2e tests
test-e2e: build-e2e-bin
	TN_BIN_PATH="$(E2E_BIN)" TN_SEED_SIGNATURE_FORK_EPOCH=$(TN_SEED_SIGNATURE_FORK_EPOCH) TN_COMMITTEE_WORKERS_FORK_EPOCH=$(TN_COMMITTEE_WORKERS_FORK_EPOCH) cargo nextest run -p e2e-tests --run-ignored ignored-only --all-features ;
