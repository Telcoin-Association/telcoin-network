...
# semantics for epochs <= 160. `faucet` also compiles in the ConsensusRegistry fork machinery
# (same arm adiri uses; see tn_types::forks) so devnet can rehearse the fork; the legacy-ABI
# pre-fork routing remains compiled into every build (keyed on the deployed registry's code hash).
docker-devnet:
