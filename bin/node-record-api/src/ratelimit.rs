...
//!
//! Copied from `bin/worker-gateway/src/ratelimit.rs`; keep the two in sync until a shared
//! `crates/tn-http-edge` is extracted (follow-up). The only local changes are the JSON error
//! envelope (`crate::error`) and the probe paths exempted from limiting (`crate::api`).
//!
//! Two token-bucket limiters shed load before a request reaches a handler: a
//! daemon-wide bucket caps aggregate throughput, and a per-client bucket keyed
//! on the client's *network prefix* caps what one source can take of that
//! budget. An over-limit request receives the daemon's JSON "rate limited"
//! error (HTTP `429`), never a bare connection reset.
//!
...
//! allocations still earns a bucket per allocation, and only the daemon-wide
//! bucket caps their total.
//!
//! The client identity is the immediate TCP peer address (`ConnectInfo`,
//! injected per connection by the accept loop). The daemon is meant to run
...
//! such a proxy's client identity upstream, or run the daemon at the edge (see
//! the crate README).
...
pub const DEFAULT_MAX_PER_IP_ENTRIES: usize = 100_000;
...
impl RateLimit {
    /// Build a limit from a sustained `rate` (requests per second) and a `burst` ceiling.
...
/// A `/32` is a single address, so it preserves the historical
...
impl PrefixLen {
    ...
    pub fn v4(bits: u8) -> Result<Self, PrefixLenError> {
        Self::checked(bits, V4_BITS)
    }

    /// An IPv6 prefix length, rejecting anything wider than `/128`.
    pub fn v6(bits: u8) -> Result<Self, PrefixLenError> {
...
impl PrefixPolicy {
    ...
    pub fn new(v4: PrefixLen, v6: PrefixLen) -> Self {
        Self { v4, v6 }
    }
...
/// The daemon's rate limiters. Either limiter may be disabled (`None`).
pub struct RateLimiters<C: Clock = SystemClock> {
    clock: C,
    global: Option<GlobalLimiter>,
    per_ip: Option<PerIpLimiter>,
}

// local addition: the type is public in this crate's library target, where the workspace's
// `missing_debug_implementations` lint applies; the clock is not required to be `Debug`
impl<C: Clock> fmt::Debug for RateLimiters<C> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
...
impl<C: Clock> RateLimiters<C> {
    ...
    /// admitted load.
    pub fn check(&self, peer: Option<IpAddr>) -> Result<(), ApiError> {
        ...
        allowed.then_some(()).ok_or(ApiError::RateLimited)
    }
...
pub async fn run_gc(limiters: Arc<RateLimiters>, shutdown: Noticer) -> Result<(), TaskError> {
...
/// request with the daemon's JSON `429` error before it reaches a handler.
pub async fn rate_limit(
    ...
    // failing under load would take the daemon down (the orchestrator kills or
    // depools the pod) exactly when it is meant to be absorbing a flood.
    let exempt = is_probe_path(request.uri().path());
...
mod tests {
    ...
    fn global_bucket_admits_burst_then_rejects() {
        ...
        assert!(limiters.check(None).is_ok());
        assert!(limiters.check(None).is_ok());
        assert!(matches!(limiters.check(None), Err(ApiError::RateLimited)));
    }
    ...
    fn rotation_inside_one_v6_prefix_shares_a_bucket() {
        ...
        assert!(
            matches!(limiters.check(Some(ip6(1, 3))), Err(ApiError::RateLimited)),
